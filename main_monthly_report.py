...
def extract_text(url):
    ...
    try:
        ...
        # /html/body/table[2]/tbody/tr/td[2]/a/img
        # /html/body/table[2]/tbody/tr/td[2]/a/img
        element.click()
        driver.implicitly_wait(2)
...
# Read Excel file of the CURRENT MONTH
skiprows = 3

file_path_current_month = r"C:\github_repos\thales_report\project\output_files\7.-july\Thales Julio 2023.xlsx" # change the path to the current file
df_current_month = pd.read_excel(file_path_current_month, skiprows=skiprows)
...
# Read Excel file of the LAST MONTH
file_path_last_month = r"C:\github_repos\thales_report\project\output_files\6.- june\june_thales_processed_data_03072023_EFV.xlsx" # change the path to the previous file
df_last_month = pd.read_excel(file_path_last_month, sheet_name='Sheet1')
...
###------------- KPIs COMPARISON ---------------------#
last_month = pd.read_excel(r"C:\github_repos\thales_report\project\output_files\6.- june\june_thales_processed_data_03072023_EFV.xlsx", sheet_name='Sheet1')
#current_month = pd.read_excel(r"C:\github-repos\thales-report\thales_march_15th_coverage_processed.xlsx")
current_month = df_current_month.copy(deep=True)
last_month = df_last_month.copy(deep=True)
...
#---
prev_cost = last_month['COSTO EURO'].sum()
...
output_dir = r"C:\github_repos\thales_report\project\output_files\7.-july"
file_path = os.path.join(output_dir, 'KPIs Comparison May - July_Monthly_EFV_31072023.txt')

with open(file_path, 'w') as file:
    file.write("VOLUME\n")
    file.write(f"June Volume: {volume_1} News\n")
    file.write(f"July volume: {volume_2} News\n")
    file.write(f"The Percentage Change Compared Last Report Is: {change}%\n\n")
    file.write("COST\n")
    file.write(f"June Total Cost: {prev_cost:.2f} Euros\n")
    file.write(f"July Total Cost: {curr_cost:.2f} Euros\n")
    file.write(f"Percentage Change Compared Last Report: {percent_change:.2f}%\n")

current_month.to_excel(r"C:\github_repos\thales_report\project\output_files\7.-july\july_thales_processed_data_31072023_EFV.xlsx", index=False, engine='xlsxwriter')
### ------------- PLOTS ---------------------#
"""
Media Plot
"""
...
tier = current_month.groupby("TIER").size().reset_index(name="counts")
...
last_costs = last_month.groupby('FECHA')['COSTO EURO'].sum().reset_index()
